...
@section('content')
    <form action="/admin/dashboard/exam/toeic/story/{{ $story->id }}" method="POST" enctype="multipart/form-data">
        @method('put')
        @csrf
        <div class="flex items-center justify-center">
            <div
                ...
                @elseif ($story->section == 'vii')
                    <div class="mt-5" id="rich-text">
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                            for="editor">Story</label>
                        <textarea id="editor" name="story">{{ $story->story }}</textarea>
                    </div>
                @endif
