...
<div class="mt-5">
    ...
    @if ($directions_vi)
        <div class="relative p-2 mt-2 border-2 border-gray-200 rounded-lg dark:border-gray-700">
            @if ($directions_vi->audio)
                <audio class="mt-2 mb-2" controls>
                    <source src="{{ asset('storage/' . $directions_vi->audio) }}">
                    Your browser does not support the audio element.
                </audio>
            @endif
            <p class="text-base text-gray-900 dark:text-white">{{ $directions_vi->direction }}</p>
            {{-- Actions --}}
            <div class="absolute top-3 right-3">
                <button type="button" data-modal-target="delete-direction-{{ $directions_vi->id }}"
                    data-modal-toggle="delete-direction-{{ $directions_vi->id }}"
                    data-bs-target="#delete-direction-{{ $directions_vi->id }}"
                ...
                <a href="/admin/dashboard/exam/toeic/direction/{{ $directions_vi->id }}/edit"
                ...
                {{-- Delete Modal --}}
                <div id="delete-direction-{{ $directions_vi->id }}" tabindex="-1"
                    class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    <div class="relative w-full max-w-md max-h-full p-4">
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            <button type="button"
                                class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                data-modal-hide="delete-direction-{{ $directions_vi->id }}">
                            ...
                            <div class="p-4 text-center md:p-5">
                                ...
                                <form class="flex justify-center "
                                    action="/admin/dashboard/exam/toeic/direction/{{ $directions_vi->id }}"
                                    method="POST">
                                    @method('delete')
                                    @csrf
                                    <button data-modal-hide="delete-direction-{{ $directions_vi->id }}" type="submit"
                                        class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center me-2">
                                        Yes, I'm sure
                                    </button>
                                    <button data-modal-hide="delete-direction-{{ $directions_vi->id }}" type="button"
