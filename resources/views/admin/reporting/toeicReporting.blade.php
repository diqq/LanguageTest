    ...
    <div class="p-5 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        ...
        <section class="mt-5">
            <table id="reporting" class="w-full text-sm text-left text-gray-500 display dark:text-gray-400">
                ...
                <tbody>
                    @foreach ($toeicScores as $toeicScore)
                        <tr class="border-b bg-gray-50 dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td scope="row" class="px-6 py-4">
                                {{ $toeicScore->user->name }}
                            </td>
                            <td scope="row" class="px-6 py-4">
                                {{ $toeicScore->score_listening }}
                            </td>
                            <td scope="row" class="px-6 py-4">
                                {{ $toeicScore->score_reading }}
                            </td>
                            <td scope="row" class="px-6 py-4">
                                {{ $toeicScore->score_total }}
                            </td>
                            <td scope="row" class="px-6 py-4">
                                {{ \Carbon\Carbon::parse($toeicScore->created_at)->translatedFormat('j F Y') }}
                            </td>
                        </tr>
                    @endforeach
                </tbody>
